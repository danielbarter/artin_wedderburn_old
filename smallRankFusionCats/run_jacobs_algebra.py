...
for line in multiplication_lines:
    ...
    j = int(parts[1])
    k = int(parts[2])
    val = complex(float(parts[3]), float(parts[4]))
    multiplication_coefficients.append(((i, j, k), val))

mult = sparse.COO.from_iter(
    multiplication_coefficients, shape=(dim, dim, dim), dtype=complex)


alg = Algebra(dim, mult, unit, is_sparse=True)

tic = time.perf_counter()
aT = ArtinWedderburn(alg)
toc = time.perf_counter()

print(dim, " ", toc - tic, " ", aT.total_defect)
